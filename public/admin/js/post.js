...
$(document).on("change", "#form-avatar1", function(evt) {
  var file = evt.target.files[0];
  ...
  if (nameExtension.search("image") > -1 && file.size < 5 * 1024 * 1024) {
    $(this)
      .next("label")
      .text(file.name);
    $("#file-show1").attr("src", tmp);
    $("#file-show1").removeClass("hidden");
  } else {
    notify("Vui lòng chọn hình có dung lượng nhỏ hơn 5MB", 0);
    $(this)
      .next("label")
      .text("Choose file");
    $("#file-show1").addClass("hidden");
    $(this).val("");
  }
});
$(document).on("change", "#form-avatar2", function(evt) {
  var file = evt.target.files[0];
  ...
  if (nameExtension.search("image") > -1 && file.size < 5 * 1024 * 1024) {
    $(this)
      .next("label")
      .text(file.name);
    $("#file-show2").attr("src", tmp);
    $("#file-show2").removeClass("hidden");
  } else {
    notify("Vui lòng chọn hình có dung lượng nhỏ hơn 5MB", 0);
    $(this)
      .next("label")
      .text("Choose file");
    $("#file-show2").addClass("hidden");
    $(this).val("");
  }
});
$(document).on("change", "#form-avatar3", function(evt) {
  var file = evt.target.files[0];
  ...
  if (nameExtension.search("image") > -1 && file.size < 5 * 1024 * 1024) {
    $(this)
      .next("label")
      .text(file.name);
    $("#file-show3").attr("src", tmp);
    $("#file-show3").removeClass("hidden");
  } else {
    notify("Vui lòng chọn hình có dung lượng nhỏ hơn 5MB", 0);
    $(this)
      .next("label")
      .text("Choose file");
    $("#file-show3").addClass("hidden");
    $(this).val("");
  }
});
